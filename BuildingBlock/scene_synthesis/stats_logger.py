...
class StatsLogger(object):
    ...
    def print_progress(self, epoch, batch, loss, time_taken=None, precision="{:.5f}"):
        self._loss.value = loss
        fmt = "epoch: {} - batch: {} - loss: " + precision
        msg = fmt.format(epoch, batch, self._loss.value)
        if time_taken is not None:
            msg += " - time: {:.4f}s".format(time_taken)
...
class WandB(StatsLogger):
    ...
    def print_progress(self, epoch, batch, loss, precision="{:.5f}", time_taken=None):
        super().print_progress(
            epoch, batch, loss, precision=precision, time_taken=time_taken
        )
...
class Swanlab(WandB):
    def init(
        ...
        if self.watch:
            swanlab.watch(model, log_freq=self.log_frequency)
