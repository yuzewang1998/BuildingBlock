...
class GaussianDiffusion:
    def __init__(
        ...
        self.iou_ignore = config.get("iou_ignore", [])
        ...
        if self.loss_iou:
            ...
            print(
                "load centriods min {} and max {} in Gausssion Diffusion".format(
            ...
            print(
                "load sizes min {} and max {} in Gausssion Diffusion".format(
                    self._sizes[0], self._sizes[1]
                )
            )
    ...
    def _predict_v(self, x0, t, eps):
        return (
            ...
            - self._extract(
                self.sqrt_one_minus_alphas_cumprod.to(x0.device), t, x0.shape
            )
            * x0
        )
    ...
    def _predict_start_from_v(self, x_t, t, v):
        return (
            ...
            - self._extract(
                self.sqrt_one_minus_alphas_cumprod.to(x_t.device), t, x_t.shape
            )
            * v
        )
    ...
    def p_mean_variance(
        ...
        preds = self.model_predictions(
            denoise_fn, data, t, condition, condition_cross, x_self_cond=None
        )
    ...
    def ddim_sample_loop(
        ...
        for time, time_next in tqdm(time_pairs, desc="sampling loop time step"):
            ...
            pred_noise, x_start, *_ = self.model_predictions(
                img, t_, condition, condition_cross, self_cond, clip_x_start=True
            )
            ...
            noise = noise_fn(
                size=shape, dtype=torch.float, device=device
    ...
    def p_sample_loop_complete(
        ...
        for t in reversed(
            ...
            img_t = torch.cat(
                [partial_boxes_t, img_t[:, num_partial:, :]], dim=1
            ).contiguous()
            ...
            if t == 0:
                ...
                img_t = torch.cat(
                    [partial_boxes, img_t[:, num_partial:, :]], dim=1
                ).contiguous()

        assert img_t.shape == shape
        return img_t

    def p_sample_loop_arrange(
        self,
        denoise_fn,
        shape,
        device,
        ...
        assert isinstance(shape, (tuple, list))
        img_t = noise_fn(
            size=(shape[0], shape[1], self.translation_dim + self.angle_dim),
            dtype=torch.float,
            device=device,
        )
        for t in reversed(
    ...
    def p_losses(
        ...
        if self.loss_type == "mse":
            ...
            if self.room_arrange_condition:
                ...
                loss_trans = (
                    (
                        target[:, :, 0 : self.translation_dim]
                        - denoise_out[:, :, 0 : self.translation_dim]
                    )
                    ** 2
                ).mean(dim=list(range(1, len(data_start.shape))))
                loss_angle = (
                    (
                        target[:, :, self.translation_dim :]
                        - denoise_out[:, :, self.translation_dim :]
                    )
                    ** 2
                ).mean(dim=list(range(1, len(data_start.shape))))
                if self.loss_separate:
                    ...
                    losses = ((target - denoise_out) ** 2).mean(
                        dim=list(range(1, len(data_start.shape)))
                    )
                losses_weight = losses * self._extract(
                    self.loss_weight.to(losses.device), t, losses.shape
                ).to(losses.device)
                ...
                loss_bbox = (
                    (
                        target[:, :, 0 : self.bbox_dim]
                        - denoise_out[:, :, 0 : self.bbox_dim]
                    )
                    ** 2
                ).mean(dim=list(range(1, len(data_start.shape))))
                loss_class = (
                    (
                        ...
                        - denoise_out[
                            :, :, self.bbox_dim : self.bbox_dim + self.class_dim
                        ]
                    )
                    ** 2
                ).mean(dim=list(range(1, len(data_start.shape))))
                ...
                if self.loss_iou:
                    ...
                    if self.objectness_dim > 0:
                        ...
                        valid_mask = (obj_recon <= 0).float().squeeze(2)  # b,class_dim
                        ...
                        for class_ignore in self.iou_ignore:
                            valid_mask -= (class_index == class_ignore).float()
                    ...
                    descale_sizes = self.descale_to_origin(
                        sizes_recon,
                        self._sizes_min.to(data_start.device),
                        self._sizes_max.to(data_start.device),
                    )
                    if self.size_half:
                        axis_aligned_bbox_corn = torch.cat(
                            [
                                descale_trans - descale_sizes,
                                descale_trans + descale_sizes,
                            ],
                            dim=-1,
                        )
                    ...
                    bbox_iou = axis_aligned_bbox_overlaps_3d(
                        axis_aligned_bbox_corn,
                        axis_aligned_bbox_corn,
                        mode=self.loss_iou_mode,
                    )

                    bbox_iou_mask = valid_mask[:, :, None] * valid_mask[:, None, :]
                    bbox_iou_valid = bbox_iou * bbox_iou_mask

                    bbox_iou_valid_avg = bbox_iou_valid.sum(
                        dim=list(range(1, len(bbox_iou_valid.shape)))
                    ) / (
                        bbox_iou_mask.sum(dim=list(range(1, len(bbox_iou_valid.shape))))
                        + 1e-6
                    )
                    # get the iou loss weight w.r.t time
                    w_iou = self._extract(
                        self.alphas_cumprod.to(data_start.device), t, bbox_iou.shape
                    )
                    loss_iou = (w_iou * 0.1 * bbox_iou).mean(
                        dim=list(range(1, len(w_iou.shape)))
                    )
                    loss_iou_valid_avg = (w_iou * 0.1 * bbox_iou_valid).sum(
                        dim=list(range(1, len(bbox_iou_valid.shape)))
                    ) / (
                        bbox_iou_mask.sum(dim=list(range(1, len(bbox_iou_valid.shape))))
                        + 1e-6
    ...
    def calc_bpd_loop(
        self, denoise_fn, x_start, condition, condition_cross, clip_denoised=True
    ):
...
class DiffusionPoint(nn.Module):
    ...
    def complete_samples(
        ...
        return self.diffusion.p_sample_loop_complete(
            self._denoise,
            shape=shape,
            device=device,
            condition=condition,
    ...
    def arrange_samples(
        ...
        return self.diffusion.p_sample_loop_arrange(
            self._denoise,
            shape=shape,
            device=device,
            condition=condition,
