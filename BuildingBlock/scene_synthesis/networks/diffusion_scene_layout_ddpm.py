...
from .denoise_net_transformer_adaln import (
    PointTransformer1D as PointTransformer1D_adaln,
)
...
class DiffusionSceneLayout_DDPM(Module):
    def __init__(self, n_classes, feature_extractor, config):
        ...
        elif config["net_type"] == "dit_adaln":
            device = "cuda" if torch.cuda.is_available() else "cpu"
            denoise_net = PointTransformer1D_adaln(
                device=device, dtype=torch.float32, **config["net_kwargs"]
            )
        else:
        ...
        self.diffusion = DiffusionPoint(
            denoise_net=denoise_net, config=config, **config["diffusion_kwargs"]
        )
        ...
        if self.learnable_embedding:
            if self.instance_condition:
                self.register_parameter(
                    ...
                    nn.Parameter(
                        torch.randn(self.sample_num_points, self.instance_emb_dim)
                    ),
                )
            else:
            ...
            if self.instance_condition:
                self.fc_instance_condition = nn.Sequential(
                    nn.Linear(
                        self.sample_num_points, self.instance_emb_dim, bias=False
                    ),
    ...
    def get_loss(self, sample_params):
        ...
        if (
            ...
            if self.objectness_dim > 0:
                ...
                room_layout_target = torch.cat(
                    [translations, sizes, angles, class_labels], dim=-1
                ).contiguous()
            if self.objfeat_dim > 0:
                room_layout_target = torch.cat(
                    [room_layout_target, objfeats], dim=-1
                ).contiguous()

        elif self.config["point_dim"] == self.bbox_dim:
            room_layout_target = torch.cat(
                [translations, sizes, angles], dim=-1
            ).contiguous()
        ...
        if self.room_mask_condition:
            ...
            room_layout_f = self.fc_room_f(
                self.feature_extractor(room_layout)
            )  # (B, F)
        else:
            room_layout_f = None
        ...
        # process instance & class condition
        # import ipdb;ipdb.set_trace()
        if self.instance_condition:
            if self.learnable_embedding:
                instance_indices = (
                    torch.arange(self.sample_num_points)
                    .long()
                    .to(device)[None, :]
                    .repeat(batch_size, 1)
                )  # B,N
                instan_condition_f = self.positional_embedding[
                    instance_indices, :
                ]  # B，N，self.instance_emb_dim
            else:
                instance_label = (
                    torch.eye(self.sample_num_points)
                    .float()
                    .to(device)[None, ...]
        ...
        if room_layout_f is not None and instan_condition_f is not None:
            condition = torch.cat(
                [
                    room_layout_f[:, None, :].repeat(1, num_points, 1),
                    instan_condition_f,
                ],
                dim=-1,
            ).contiguous()
        ...
        if self.room_partial_condition:
            partial_valid = (
                torch.ones((batch_size, self.partial_num_points, 1)).float().to(device)
            )
            partial_invalid = (
                torch.zeros((batch_size, num_points - self.partial_num_points, 1))
                .float()
                .to(device)
            )
            partial_mask = torch.cat(
                [partial_valid, partial_invalid], dim=1
            ).contiguous()
        ...
        if self.room_arrange_condition:
            ...
            room_layout_target = torch.cat(
                [
                    ...
                    room_layout_target[
                        :, :, self.translation_dim + self.size_dim : self.bbox_dim
                    ],
                ],
                dim=-1,
            ).contiguous()
        ...
        if self.text_condition:
            if self.text_glove_embedding:
                ...
                tokenized = self.tokenizer(
                    sample_params["description"], return_tensors="pt", padding=True
                ).to(device)
                if tokenized.input_ids.shape[1] > 512:
                    # print("warnning!! token exceed 512")
                    for k in tokenized.keys():
                        tokenized[k] = tokenized[k][..., :512]
    ...
    def forward(self, sample_params):
        return self.get_loss(sample_params)
    ...
    def sample(
        ...
        if self.room_partial_condition:
            zeros_boxes = (
                torch.zeros(
                    (
                        batch_size,
                        num_points - partial_boxes.shape[1],
                        partial_boxes.shape[2],
                    )
                )
                .float()
                .to(device)
        ...
        if self.text_condition:
            if self.text_glove_embedding:
                ...
                tokenized = self.tokenizer(text, return_tensors="pt", padding=True).to(
                    device
                )
        ...
        if input_boxes is not None:
            ...
            samples = self.diffusion.arrange_samples(
                noise.shape,
                room_mask.device,
                condition=condition,
            ...
            samples = self.diffusion.complete_samples(
                noise.shape,
                room_mask.device,
                condition=condition,
                condition_cross=condition_cross,
    ...
    def generate_layout(
        ...
        return self.delete_empty_from_network_samples(
            samples, device=device, keep_empty=keep_empty
        )
    ...
    def generate_layout_progressive(
        ...
        for i in range(len(samples_traj)):
            ...
            k_time = num_step * i
            boxes_traj[k_time] = self.delete_empty_from_network_samples(
                samples, device=device, keep_empty=keep_empty
            )
        return boxes_traj
    ...
    @torch.no_grad()
    def delete_empty_from_network_samples(
        self, samples, device="cpu", keep_empty=False
    ):
        # import ipdb;ipdb.set_trace()
        samples_dict = {
            ...
            "class_labels": nn.functional.one_hot(
                torch.argmax(
                    samples[
                        :, :, self.bbox_dim : self.bbox_dim + self.class_dim - 1
                    ].contiguous(),
                    dim=-1,
                ),
            ...
            "objectness": samples[
                :,
                :,
                self.bbox_dim + self.class_dim - 1 : self.bbox_dim + self.class_dim,
            ]
            >= 0,
        }
        if self.objfeat_dim > 0:
        ...
        def empty_box():
            return {
                ...
                "objfeats": (
                    torch.zeros(1, 0, self.objfeat_dim, device=device)
                    if self.objfeat_dim > 0
                    else None
                ),
            }
        ...
        if batch == 1:
            ...
            boxes = [empty_box() for _ in range(batch)]
        ...
        if batch == 1:
                ...
                if not keep_empty and samples_dict["objectness"][0, i, -1] > 0:
                    ...
                    for k in samples_dict.keys():
                        if k == "class_labels":
                            ...
                            boxes[k] = torch.cat(
                                [boxes[k], samples_dict[k][:, i : i + 1, :].to(device)],
                                dim=1,
                            )
            if self.objfeat_dim > 0:
                return {
            ...
            for batch_i in range(batch):
                for i in range(max_boxes):
                    ...
                    if (
                        not keep_empty
                        and samples_dict["objectness"][batch_i, i, -1] > 0
                    ):
                        continue
                    else:
                        for k in samples_dict.keys():
                            if k == "class_labels":
                                ...
                                boxes[batch_i]["objectness"] = torch.cat(
                                    [
                                        boxes[batch_i]["objectness"],
                                        samples[
                                            batch_i : batch_i + 1,
                                            i : i + 1,
                                            self.bbox_dim
                ...
                for k in samples_dict.keys():
                    if k == "class_labels":
                        ...
                        boxes[k] = torch.cat(
                            [
                                boxes[k],
                                samples_dict[k][:, i : i + 1, : self.class_dim - 1].to(
                                    device
                                ),
                            ],
                            dim=1,
                        ...
                        boxes["objectness"] = torch.cat(
                            [
                                boxes["objectness"],
                                samples_dict[k][:, i : i + 1, -1:].to(device),
                            ],
                            dim=1,
                        )
                    else:
...
def train_on_batch_ddp(model, optimizer, sample_params, config):
    # Make sure that everything has the correct size
    optimizer.zero_grad()

    loss, loss_dict = model(sample_params)
    loss = loss.mean()

    for k, v in loss_dict.items():
        StatsLogger.instance()[k].value = v.mean().item()
    # Do the backpropagation
    loss.backward()
    # Compuite model norm
    grad_norm = clip_grad_norm_(
        model.module.parameters(), config["training"]["max_grad_norm"]
    )
    StatsLogger.instance()["gradnorm"].value = grad_norm.item()
    # log learning rate
    StatsLogger.instance()["lr"].value = optimizer.param_groups[0]["lr"]
