...
from .building_blocks import BuildingBlocks, CachedBuildingBlocks
...
def get_raw_dataset(
    ...
    if "cached" in dataset_type:
        ...
        if "building" in dataset_type:
            dataset = BuildingBlocks.from_dataset_directory(
                config["dataset_directory"],
...
def get_encoded_dataset(
    ...
    _, encoding = get_dataset_raw_and_encoded(
        config, filter_fn, path_to_bounds, augmentations, split, data_cache
    )
    return encoding
...
def filter_function(config, split=["train", "val"], without_lamps=False):
    ...
    livingroom_common_filters = [
        BaseDataset.at_least_boxes(3),
        BaseDataset.at_most_boxes(21),
        BaseDataset.with_object_types(list(THREED_FRONT_LIVINGROOM_FURNITURE.keys())),
    ...
    if "threed_front_bedroom" in config["filter_fn"]:
        ...
        # 活用 * 操作符将列表展开为函数参数
        return BaseDataset.filter_compose(
            BaseDataset.with_room("living"),  # 特定于客厅的过滤器
            *livingroom_common_filters
        )
    elif "threed_front_diningroom" in config["filter_fn"]:
        return BaseDataset.filter_compose(
            BaseDataset.with_room("dining"),  # 特定于餐厅的过滤器
            *livingroom_common_filters
        )
