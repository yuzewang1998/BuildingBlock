...
class CachedThreedFront(ThreedFront):
    def __init__(self, base_dir, config, scene_ids, cache_dist={}):
    ...
    def get_room_params(self, i):
        # if i in CachedThreedFront._cache:
        #     return deepcopy(CachedThreedFront._cache[i])
        if i in self._cache:
            return deepcopy(self._cache[i])
        D = np.load(self._path_to_rooms[i])
        ...
        self._cache[i] = deepcopy(data_dict)
        # CachedThreedFront._cache[i] = deepcopy(data_dict)
        return data_dict
