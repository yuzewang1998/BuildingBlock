...
class RotationAugmentation(DatasetDecoratorBase):
    def __init__(
        ...
        self._fixed = fixed
        self._without_rotate = without_rotate
        self._cache = cache
    ...
    @property
    def without_rotate(self):
        return 0.0

    def __getitem__(self, idx):
        # Get the rotation matrix for the current scene
        # print("cache len: ",len(self._cache))
        if self._fixed:
            rot_angle = self.fixed_rot_angle
        else:
            rot_angle = self.rot_angle

        if self._without_rotate:
            rot_angle = self.without_rotate

        if (idx, round(rot_angle, 2)) in self._cache:
            # print("cache hit")
            return deepcopy(self._cache[(idx, round(rot_angle, 2))])
        # else:
        #     print("cache {} miss".format((idx, round(rot_angle,2))))
        ...
        self._cache[(idx, round(rot_angle, 2))] = deepcopy(sample_params)
        return sample_params
...
class Add_Text_Building(DatasetDecoratorBase):
    def __init__(self, dataset):
        super().__init__(dataset)
...
class Autoregressive(DatasetDecoratorBase):
    def __getitem__(self, idx):
        ...
        if "texts" in sample_params.keys():
            del sample_params["texts"]
        for k, v in sample_params.items():
...
class Diffusion(DatasetDecoratorBase):
    def __getitem__(self, idx):
        ...
        pad_zero = self._dataset.pad_zero

        # Add the number of bounding boxes in the scene
        sample_params["length"] = sample_params["class_labels"].shape[0]

        if sample_params["length"] > max_length:
            return self.__getitem__(random(0, len(self._dataset)))
        ...
        for k, v in sample_params.items():
            if k == "room_layout" or k == "length":
                ...
                if pad_zero:
                    ...
                    if inds is None:
                        inds = np.random.choice(L, max_length - L)
                    sample_params_target[k] = np.vstack([p, p[inds]]).astype(np.float32)
...
class DiffusionPermuteAll(Diffusion):
    def __init__(self, dataset, permutation_keys, permutation_axis=0):
        super().__init__(dataset)
        self._permutation_keys = permutation_keys
        self._permutation_axis = permutation_axis
    ...
    def __getitem__(self, idx):
        ...
        if sample_params["length"] > max_length:
            return self.__getitem__(random.randint(0, len(self._dataset) - 1))
...
def dataset_encoding_factory(
    name, dataset, augmentations=None, box_ordering=None, data_cache={}
):
    ...
    if isinstance(augmentations, list):
        for aug_type in augmentations:
            if aug_type == "rotations":
                ...
                dataset_collection = RotationAugmentation(
                    dataset_collection, fixed=True, cache=data_cache
                )
        ...
        # just use cache
        dataset_collection = RotationAugmentation(
            dataset_collection, without_rotate=True, cache=data_cache
        )
    ...
    elif "txtBuilding" in name:
        print("add text into input dict for training")
        dataset_collection = Add_Text_Building(dataset_collection)
    ...
    if "diffusion" in name:
        ...
        elif "wocm_prmAll" in name:
            return DiffusionPermuteAll(
                dataset_collection,
                permute_keys,
            )
        elif "wocm" in name:
