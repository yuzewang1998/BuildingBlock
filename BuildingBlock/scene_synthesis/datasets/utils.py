...
def parse_threed_front_scenes(
    ...
    if os.getenv("PATH_TO_SCENES"):
        ...
        scenes = sum(scenes, [])
        pickle.dump(
            scenes, open("../3d_front_processed/threed_front.pkl", "wb")
        )  # /tmp/threed_front.pkl
...
def parse_threed_future_models(dataset_directory, path_to_models, path_to_model_info):
    if os.getenv("PATH_TO_3D_FUTURE_OBJECTS"):
        ...
        pickle.dump(
            furnitures, open("../3d_front_processed/threed_future_model.pkl", "wb")
        )  # /tmp/threed_future_model.pkl
