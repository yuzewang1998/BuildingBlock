    ...
    for j in range(start, end):
        ...
        if no_texture:
            ...
            tr_mesh.visual.vertex_colors = (
                color[None, :].repeat(tr_mesh.vertices.shape[0], axis=0).reshape(-1, 3)
                * 255.0
            ).astype(np.uint8)
            tr_mesh.visual.face_colors = (
                color[None, :].repeat(tr_mesh.faces.shape[0], axis=0).reshape(-1, 3)
                * 255.0
            ).astype(np.uint8)
        else:
            tr_mesh.visual.material.image = Image.open(furniture.texture_image_path)
            tr_mesh.visual.vertex_colors = (tr_mesh.visual.to_color()).vertex_colors[
                :, 0:3
            ]
...
def get_textured_objects_based_on_objfeats(
    bbox_params_t,
    objects_dataset,
    classes,
    diffusion=False,
    ...
    for j in range(start, end):
        ...
        raw_bbox_vertices = np.load(
            furniture.path_to_bbox_vertices, mmap_mode="r"
        )  # np.array(raw_mesh.bounding_box.vertices)
        raw_sizes = np.array(
            [
