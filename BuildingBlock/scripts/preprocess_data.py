...
def main(argv):
    ...
    # add objfeat
    parser.add_argument(
        "--add_objfeats", action="store_true", help="if remove the floor plane"
    )
    ...
    if args.add_objfeats:
        ...
        print(
            [of_bounds[0], of_bounds[1], of_bounds[2]],
            type(of_bounds[0]),
            of_bounds[0].shape,
        )
        dataset_stats["bounds_objfeats"] = (
            of_bounds[0].tolist() + of_bounds[1].tolist() + of_bounds[2].tolist()
        )
        print(of_bounds[0].tolist() + of_bounds[1].tolist() + of_bounds[2].tolist())
        print(
            "add objfeats statistics: std {}, min {}, max {}".format(
                of_bounds[0], of_bounds[1], of_bounds[2]
            )
        )
        ...
        print(
            [of_bounds_32[0], of_bounds_32[1], of_bounds_32[2]],
            type(of_bounds_32[0]),
            of_bounds_32[0].shape,
        )
        dataset_stats["bounds_objfeats_32"] = (
            of_bounds_32[0].tolist()
            + of_bounds_32[1].tolist()
            + of_bounds_32[2].tolist()
        )
        print(
            of_bounds_32[0].tolist()
            + of_bounds_32[1].tolist()
        ...
        print(
            "add objfeats_32 statistics: std {}, min {}, max {}".format(
                of_bounds_32[0], of_bounds_32[1], of_bounds_32[2]
            )
        )
    ...
    for (i, es), ss in tqdm(zip(enumerate(encoded_dataset), dataset)):
        ...
        # if '6295' not in room_directory: continue
        ...
        with DirLock(room_directory + ".lock") as lock:
            ...
            if args.no_texture:
                ...
                color_palette = np.array(
                    sns.color_palette("hls", class_labels.shape[1] - 2)
                )
