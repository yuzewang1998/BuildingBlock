...
from scene_synthesis.networks.loss import axis_aligned_bbox_overlaps_3d
...
def merge_meshes(meshes):
    ...
    for i in range(len(meshes)):
        ...
        vertices[vertex_offset : vertex_offset + current_vertices.shape[0]] = (
            current_vertices
        )
        triangles[triangle_offset : triangle_offset + current_triangles.shape[0]] = (
            current_triangles + vertex_offset
        )
        vertex_colors[
            vertex_color_offset : vertex_color_offset + current_vertex_colors.shape[0]
        ] = current_vertex_colors[:, 0:3]
...
def render_to_folder(
    ...
    if args.save_mesh:
        ...
        if args.no_texture:
            ...
            path_to_scene = os.path.join(
                args.output_directory, folder, filename + args.mesh_format
            )
    ...
    path_to_image = os.path.join(
        args.output_directory, folder, filename + "_render.png"
    )
...
def render_scene_from_bbox_params(
    ...
    if args.retrive_objfeats:
        ...
        if args.retrive_surface:
            ...
            with torch.no_grad():
                decoded_surf = network_objae.decode(
                    boxes["objfeats"].to(device).float().reshape(B * N, C)
                )
...
def computer_intersection(trimeshes, judge_mesh_intersec=False):
    ...
    return (
        len(trimeshes),
        len(iou_list),
        float(sum(iou_list)) / len(iou_list),
        float(sum(insec_list)) / len(iou_list),
        overlap_ratio.item(),
    )
...
def judge_if_symmetry(box1, box2, size_diff=0.1, pos_diff=0.1):
    ...
    if np.abs(size1 - size2).max() < size_diff:
        if (
            abs(center1[0] - center2[0]) < pos_diff
            or abs(center1[2] - center2[2]) < pos_diff
        ):
            return True
        else:
            return False
        ...
        for i in range(len(trimeshes)):
            ...
            for j in range(i + 1, len(trimeshes)):
                ...
                if model_jids is not None:
                    ...
                    if (
                        class1 == class2
                        and num_verts_list[i] == num_verts_list[j]
                        and num_faces_list[i] == num_faces_list[j]
                    ):
                        if judge_if_symmetry(box1, box2):
                            num_symmetry += 1
