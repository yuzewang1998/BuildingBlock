...
from training_utils import load_config, yield_forever
...
def main(argv):
    ...
    for i in range(args.n_sequences):
        ...
        if i != args.n_sequences - 1 and len(batch_sample) < val_batch:
            continue
        ...
        room_masks = torch.cat(room_masks, 0) if len(room_masks) > 1 else room_masks[0]
        ...
        for samples, current_scene, boxes, it, scene_idx in zip(
            ...
            for i in range(boxes["class_labels"].shape[1]):
                class_label = classes[
                    boxes["class_labels"][0][i].argmax(-1).cpu().numpy()
                ]
                # print('class label:', class_label)
                length = boxes["sizes"][0][i][0].cpu().numpy()
                width = boxes["sizes"][0][i][1].cpu().numpy()
                height = boxes["sizes"][0][i][2].cpu().numpy()
                # print('size:',length,width,height)
                angle = (
                    round(float(torch.rad2deg(boxes["angles"][0][i][0].cpu()))) % 360
                )
                # print('angle:',angle)
                left = (
                    boxes["translations"][0][i][0].cpu().numpy()
                    - 1 / 2 * length
                    - (-2.7625005)
                    + 0.5 * (2.8682)
                )
                top = (
                    boxes["translations"][0][i][1].cpu().numpy() + 1 / 2 * width
                )  # -(0.045) #+0.5*(1.770065)
                depth = (
                    boxes["translations"][0][i][2].cpu().numpy()
                    - 1 / 2 * height
                    - (-2.75275)
                    + 0.5 * (1.698315)
                )
                # print('location:',left,top,depth)

                # "FURNITURE {{length: ?px; width: ?px; height: ?px; orientation: ? degrees; left: ?px; top: ?px; depth: ?px;}}\n"
                print(
                    f"Object {i}: {class_label.capitalize()} {{length: {int(length*100)}px; width: {int(width*100)}px; height: {int(height*100)}px; orientation: {angle} degrees; left: {int(left*100)}px; top: {int(top*100)}px; depth: {int(depth*100)}px;}}\n"
                )
            ...
            if args.render_top2down:
                path_to_image = "{}/{}_{}_{:03d}.png".format(
                    args.output_directory, current_scene.scene_id, scene_idx, it
                )
                render_top2down(
                    scene_top2down,
                    renderables,
